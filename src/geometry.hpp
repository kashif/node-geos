 ...
 */
#define GEOS_UNARY_TOPOLOGY(cppmethod, jsmethod, geosfn)                            \
 ...
 */
#define GEOS_UNARY_TOPOLOGY_TOLERANCE(cppmethod, jsmethod, geosfn)                  \
 ...
 * topology functions which return a new geometry.
 */
#define GEOS_BINARY_TOPOLOGY(cppmethod, jsmethod, geosfn)                           \
    Handle<Value> Geometry::cppmethod(const Arguments& args)                        \
    {                                                                               \
        HandleScope scope;                                                          \
        if (args.Length() != 1)                                                     \
 ...
 * predicate functions.
 */
#define GEOS_UNARY_PREDICATE(cppmethod, jsmethod, geosfn)               \
    Handle<Value> Geometry::cppmethod(const Arguments& args)            \
    {                                                                   \
        Geometry *geom = ObjectWrap::Unwrap<Geometry>(args.This());     \
        HandleScope scope;                                              \
 ...
 * predicate functions.
 */
#define GEOS_BINARY_PREDICATE(cppmethod, jsmethod, geosfn)                  \
    Handle<Value> Geometry::cppmethod(const Arguments& args)                \
    {                                                                       \
        Geometry *geom = ObjectWrap::Unwrap<Geometry>(args.This());         \
        HandleScope scope;                                                  \
 ...
 */
#define GEOS_BINARY_PREDICATE_TOLERANCE(cppmethod, jsmethod, geosfn)                    \
    Handle<Value> Geometry::cppmethod(const Arguments& args)                            \
    {                                                                                   \
        Geometry *geom = ObjectWrap::Unwrap<Geometry>(args.This());                     \
        HandleScope scope;                                                              \
 ...
 */
#define GEOS_PREPARED_GEOM_PREDICATE(cppmethod, jsmethod, geosfn)           \
    Handle<Value> Geometry::cppmethod(const Arguments& args)                \
    {                                                                       \
        Geometry *geom = ObjectWrap::Unwrap<Geometry>(args.This());         \
        HandleScope scope;                                                  \
